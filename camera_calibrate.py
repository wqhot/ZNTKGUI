...
from PyQt5.QtWidgets import QListWidget, QWidget, QApplication, QListView, QHBoxLayout, QListWidgetItem, QDialog
...
class camCalibrateUtil(QThread):
    ...
    def run(self):
        # 精准化角点迭代终止条件
        ctiteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, self.checkboard_size, 1e-6)
        ...
        while True:
            ...
            # 寻找棋盘格点
            ok, corners = cv2.findChessboardCorners(gray, (self.corner_num_x, self.corner_num_y))
            if ok:
                # 获取更精确的角点
                ...
                if len(self.imgpoints) > 0:
                    ...
                    frame = cv2.fisheye.undistortImage(frame, K, D)
...
class camviewDialog(QDialog, Ui_Dialog):
    def __init__(self):
        ...
        self.cam = camCalibrateUtil(cap, 27, 9, 6)
